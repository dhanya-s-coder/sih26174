            ...
            for obj in objects:
                if self._check_contact(hand, obj):
                    ...
                    if key not in self.contact_state:
                        self.contact_state[key] = {"frames": 0, "active": False, "start_time": time.monotonic(), "release_count": 0}
                    ...
                    if state["frames"] >= self.min_frames and not state["active"]:
                        state["active"] = True
                        # Use wall-clock monotonic time for live webcams. Their
                        # CAP_PROP_POS_MSEC timestamp is often zero/unreliable.
